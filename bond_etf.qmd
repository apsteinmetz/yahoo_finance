...
```
## Constructing Yield Curve trades with ETFs

In this note we explore how to construct yield curve trades using bond ETFs. We will use the iShares 1-3 Year Treasury Bond ETF (SHY), the iShares 7-10 Year Treasury Bond ETF (IEF), and the iShares 20+ Year Treasury Bond ETF (TLT). We will look at the historical prices, returns, and volatility of these ETFs, and then construct a hedged portfolio using different hedge ratios based on duration, beta and volatility.

A key goal of any spread trade, or any trade at all, is not make any unintended side bets.  We want to isolate the yield curve trade and not have any exposure to the general movement in interest rates.  For this trade is means looking at the price movement of our ETF's relative to each other over a periods of both curve steepening and flattening and relative to equal yield moves.

We can download the closing and "adjusted" prices of these ETFs from Yahoo Finance. "Adjusted" means dividends are include so we can calculate the total return. We get yield curve data the Federal Reserve Economic Data (FRED) database.

Let's explore the properties of the ETFs and the yield curve data.

```{r # plot price history}
# plot price history
...
plot_prices()
```
SHY is the iShares 1-3 Year Treasury Bond ETF, IEF is the iShares 7-10 Year Treasury Bond ETF, and TLT is the iShares 20+ Year Treasury Bond ETF. You can see how the longer maturity ETFs (IEF and TLT) have much more price volatility, though they are correlated.

At first glance the "round trip" the prices makes suggest own them over this period was not a good idea. But the chart above uses closing prices. Let's look at the cumulative returns using the adjusted prices, which include dividends.

```{r ggplot SHY close and adjusted prices}
...
That's better. The reward for higher volatility is more income and, over this period, better return. The relationship is inverted when the yield curve inverts.
...
```{r # plot rolling vol}
plot_rolling_volatility <- function(data = returns) {
  data |>
